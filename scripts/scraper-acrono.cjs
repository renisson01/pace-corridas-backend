#!/usr/bin/env node
/**
 * REGENI — Scraper ACrono Esportes (Wiclax CLAX)
 * acronoesportes.com.br — Mato Grosso / triathlon e corridas
 *
 ...
 *
 * Uso:
 *   node scripts/scraper-acrono.cjs
 *   node scripts/scraper-acrono.cjs --dry-run
 *   node scripts/scraper-acrono.cjs --limit 5
 */
'use strict';
...
const BASE          = 'https://acronoesportes.com.br/resultados/';
const LISTING_URL   = 'https://acronoesportes.com.br/resultados-eventos';
const ORGANIZER     = 'ACrono';
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = lib.get(url, {
      headers: {
        ...
        'Accept': '*/*',
        'Referer': 'https://acronoesportes.com.br/',
      },
      timeout: 30000,
    }, res => {
...
async function importEvent(db, claxPath, parsed) {
  ...
  if (ex.rows.length) {
    ...
    await db.query(
      ...
      [raceId, name, 'Mato Grosso', DEFAULT_STATE, dateStr, distStr, ORGANIZER, 'completed']
    );
  }
