#!/usr/bin/env node
/**
 * REGENI — Scraper TimeCrono (Wiclax CLAX/XML)
 * timecrono.com.br — Pernambuco/Nordeste
 *
 * Formato: XML Wiclax (.clax) — mesmo padrão do CronosChip
 *   <E d="dossard" n="nome" a="anoNasc" x="M/F" ca="categoria" p="distância" />
 *   <R d="dossard" t="00h28'24,8" m="03:20" />
 *
 * Uso:
 *   node scripts/scraper-timecrono.cjs
 *   node scripts/scraper-timecrono.cjs --dry-run
 *   node scripts/scraper-timecrono.cjs --limit 5
 */
'use strict';
...
const BASE          = 'https://www.timecrono.com.br/resultados/';
const LISTING_URL   = 'https://timecrono.com.br/resultados-eventos';
const ORGANIZER     = 'TimeCrono';
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = lib.get(url, {
      headers: {
        ...
        'Accept': '*/*',
        'Referer': 'https://timecrono.com.br/',
      },
      timeout: 30000,
    }, res => {
...
function parseClaxUrls(html) {
  const urls = new Set();
  // Captura tanto "/" quanto "\/" (backslash-escaped em JSON inline)
  const re = /g-live\.html\?f=(evento[^\s'"&<>]+?\.clax)/g;
  let m;
  while ((m = re.exec(html)) !== null) {
    // Remove backslashes: "evento\/2026\/..." → "evento/2026/..."
...
function parseClax(xml) {
  ...
  // Parse atletas <E> — atributo "a" (ano nasc) é opcional em alguns eventos
  const athletes = {};
  const eRe = /<E\s[^>]*d="(\d+)"[^>]*>/g;
  let em;
...
async function importEvent(db, claxPath, parsed) {
  ...
  if (ex.rows.length) {
    ...
    await db.query(
      ...
      [raceId, name, 'Pernambuco', DEFAULT_STATE, dateStr, distStr, ORGANIZER, 'completed']
    );
  }
