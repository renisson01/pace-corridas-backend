#!/usr/bin/env node
/**
 * REGENI — Scraper TriChip Cronometragem (Wiclax CLAX)
 * trichip.com.br — Rio Grande do Sul / Porto Alegre
 *
 * Listagem: trichip.com.br (514 eventos)
 * CLAX base: www.trichipcronometragem.com.br/resultados/
 *
 * Formato: XML Wiclax (.clax)
 *   <E d="dossard" n="nome" a="anoNasc" x="M/F" ca="categoria" p="distância" />
 *   <R d="dossard" t="00h28'24,8" m="03:20" />
 *
 * Nota: R tags ficam no FINAL do arquivo — deve-se baixar completo.
 *
 * Uso:
 *   node scripts/scraper-trichip.cjs
 *   node scripts/scraper-trichip.cjs --dry-run
 *   node scripts/scraper-trichip.cjs --limit 10
 */
'use strict';
...
const BASE          = 'https://www.trichipcronometragem.com.br/resultados/';
const LISTING_URL   = 'https://trichip.com.br/resultados-eventos-trichip';
const ORGANIZER     = 'TriChip';
...
// ─── Parse página de listagem → URLs CLAX ────────────────────────────────────
...
function parseClax(xml) {
  ...
  // Converter data Wiclax PT → YYYY-MM-DD (também tenta dt1 attr)
  const meses = {janeiro:1,fevereiro:2,março:3,marco:3,abril:4,maio:5,junho:6,
                 julho:7,agosto:8,setembro:9,outubro:10,novembro:11,dezembro:12};
  let date = null;

  // Tentar dt1="2026-04-26" primeiro
  const dt1 = attr(evTag, 'dt1');
  if (dt1 && /^\d{4}-\d{2}-\d{2}$/.test(dt1)) date = dt1;
...
function fmtTime(raw) {
  ...
  return `${String(h).padStart(2,'0')}:${String(min).padStart(2,'0')}:${String(s).padStart(2,'00')}`;
}
...
async function importEvent(db, claxPath, parsed) {
  ...
  if (ex.rows.length) {
    ...
    await db.query(
      ...
      [raceId, name, 'Porto Alegre', DEFAULT_STATE, dateStr, distStr, ORGANIZER, 'completed']
    );
  }
