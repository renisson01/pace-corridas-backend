...
app.get('/scraper/status', async()=>{ const [races,results,athletes]=await Promise.all([prisma.race.count(),prisma.result.count(),prisma.athlete.count()]); return{totalRaces:races,totalResults:results,athletes,status:'online'}; });

setTimeout(async()=>{ try{ const{runScraperJob}=await import('./jobs/scraperJob.js'); runScraperJob(); setInterval(()=>runScraperJob(),4*60*60*1000); }catch(e){console.error('[CRON]',e.message);} },60000);

app.listen({port:process.env.PORT||3000,host:'0.0.0.0'},(err)=>{ if(err){console.error(err);process.exit(1);} console.log('✅ PACE online'); });
