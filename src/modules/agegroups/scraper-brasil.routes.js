...
export default async function scraperBrasilRoutes(fastify) {
  ...
  fastify.get('/scraper/status', async (req,reply) => {
    const [totalRaces,totalResults,athletes] = await Promise.all([
      prisma.race.count(), prisma.result.count(), prisma.athlete.count()
    ]);
  ...
  fastify.get('/races/:id/top5', async (req,reply) => {
    ...
    where.time = { not: '00:00:00' };
    const all=await prisma.result.findMany({where,include:{athlete:{select:{name:true,gender:true,state:true}}},orderBy:{time:'asc'},take:20});
    ...
    return {race:race?.name,distance,masculino:masc.map((r,i)=>({pos:i+1,nome:r.athlete?.name,cidade:r.athlete?.state||'',tempo:r.time,pace:r.pace,faixa:r.ageGroup})),feminino:fem.map((r,i)=>({pos:i+1,nome:r.athlete?.name,cidade:r.athlete?.state||'',tempo:r.time,pace:r.pace,faixa:r.ageGroup}))};
  });
  ...
  fastify.get('/atleta/:id', async (req,reply) => {
    ...
    return {
      id: athlete.id, name: athlete.name, gender: athlete.gender, age: athlete.age,
      state: athlete.state, equipe: athlete.equipe,
      totalRaces: athlete.results.length, totalPoints: pts, nivel,
      linkedUserId: athlete.user?.id || null,
      melhoresPorDist,
      provas: athlete.results.map(r => ({
        raceName: r.race?.name || 'Corrida', raceDate: r.race?.date, raceCity: r.race?.city,
        distance: r.distance, time: r.time, pace: r.pace,
        overallRank: r.overallRank, genderRank: r.genderRank, ageGroup: r.ageGroup, points: r.points
      }))
    };
  });
