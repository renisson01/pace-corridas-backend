  ...
  fastify.post('/cobaia/checkin', async (req, reply) => {
    ...
    try {
      const checkin = await prisma.cobaiaDiario.upsert({
        ...
        create: { userId: u.userId, data: hoje, peso, gorduraPct, massaMagra, hrvMedia, fcRepouso, horasSono, qualidadeSono, horasDormir, horasAcordar, humor, energia, vyvanse: vyvanse || false, notas },
        update: { peso, gorduraPct, massaMagra, hrvMedia, fcRepouso, horasSono, qualidadeSono, horasDormir, horasAcordar, humor, energia, vyvanse: vyvanse || false, notas }
      });
  ...
  fastify.post('/cobaia/alimentacao', async (req, reply) => {
    ...
    try {
      const reg = await prisma.cobaiaAlimentacao.create({
        data: { userId: u.userId, data: new Date(), refeicao, descricao, fotoUrl, caloriasEst, proteinaEst, carbEst, gorduraEst, horario }
      });
  ...
  fastify.get('/cobaia/dashboard/:userId?', async (req, reply) => {
    ...
    try {
      ...
      const score = calcularScore({
        horasSono: ultimo.horasSono || 0,
        hrvMedia: ultimo.hrvMedia || 0,
        fcRepouso: ultimo.fcRepouso || 60,
        gorduraPct: ultimo.gorduraPct || 15,
        treinouHoje: treinos.some(t => {
          const d = new Date(t.iniciadoEm); d.setHours(0,0,0,0);
          return d.getTime() === hoje.getTime();
        }),
        streak
      });
      const bioAge = calcularIdadeBiologica(user?.age || 31, score);
