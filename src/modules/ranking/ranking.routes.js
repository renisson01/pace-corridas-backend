...
export async function rankingRoutes(fastify) {
  ...
  fastify.get('/ranking', async (req) => {
    ...
    const atletas = await prisma.athlete.findMany({
      ...
      select: { id:true, name:true, equipe:true, state:true, gender:true, totalRaces:true, totalPoints:true, age:true,
        results: { orderBy: { points: 'desc' }, take: 1, include: { race: { select: { name:true } } } }
      }
    });
    return atletas.map((a, i) => {
      const n = nivelAtleta(a.totalPoints);
      return { posicao: i+1, id:a.id, name:a.name, equipe:a.equipe||null, state:a.state||null, gender:a.gender, totalRaces:a.totalRaces, totalPoints:a.totalPoints, nivel:n.label, nivelCor:n.cor, melhorProva:a.results[0]?.race?.name||null };
    });
  });
  ...
  fastify.get('/buscar-atletas', async (req) => {
    ...
    return atletas.map((a, i) => ({
      posicao: i+1, id:a.id, name:a.name, equipe:a.equipe||null, state:a.state||null,
      gender:a.gender, totalPoints:a.totalPoints, totalRaces:a.totalRaces,
      nivel:nivelAtleta(a.totalPoints).label, melhorProva:a.results[0]?.race?.name||null
    }));
  });
...
async function rankingPorDistancia(distKm, genero) {
  ...
  const atletas = await prisma.athlete.findMany({
    where,
    select: { id:true, name:true, equipe:true, state:true, gender:true, totalPoints:true,
  ...
  return atletas
    ...
    .map(a => {
      const melhor = a.results.reduce((best, r) => tempoParaSegundos(r.time) < tempoParaSegundos(best.time) ? r : best);
      return { id:a.id, name:a.name, equipe:a.equipe||null, state:a.state||null, gender:a.gender, totalPoints:a.totalPoints, nivel:nivelAtleta(a.totalPoints).label, melhorTempo:melhor.time };
    })
